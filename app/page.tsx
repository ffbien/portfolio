...
const projects: Project[] = [
  {
    heroLabel: 'AICG',
    slug: 'ziyou-anpai', number: '01', brand: '个人创作', type: 'AIGC 短片', title: '自有安排', titleEn: 'Plans of Her Own', category: 'TV & FILM', video: siteAsset('/portfolio-media/ziyou-anpai.mp4'), poster: siteAsset('/portfolio-media/ziyou-anpai.jpg'), year: '2026年', role: '创意 / AIGC 制作 / 剪辑师', description: '《自有安排》是一部生活观察式 AIGC 短片。62 岁的王爱群习惯替亲近的人拿主意。她挑了一条“最能活”的鱼放生，直到清理河道的新闻让她开始怀疑自己的安排。项目从故事、剧本、分镜、资产建立，一直推进到生成与最终剪辑。\n\n本片限定制作预算为 99 美元，因此是在有限预算内完成的，部分画面细节仍有不足。',
    tools: [
      { label: '制作平台', value: 'Higgsfield' },
      { label: '图像模型', value: 'GPT Image 2.5' },
      { label: '视频模型', value: 'Seedance 2.5' },
    ],
    process: [
      { title: '场景细化', caption: '保留住宅生活感与叙事道具', image: siteAsset('/portfolio-media/ziyou-anpai-process-scene.png') },
      { title: '故事梗概', caption: '从人物盲点建立四场结构', image: siteAsset('/portfolio-media/ziyou-anpai-process-story.png') },
      { title: '成片关键帧', caption: '按真实时码核对镜头连续性', image: siteAsset('/portfolio-media/ziyou-anpai-process-frames.png') },
      { title: '人物资产', caption: '统一角色外观、服装与表演状态', image: siteAsset('/portfolio-media/ziyou-anpai-process-character.png') },
    ],
    dossier: siteAsset('/portfolio-media/ziyou-anpai-project-dossier.pdf'),
  },
  {
    slug: 'dian-shi-making-of-gan-wei-ren-xian', number: '02', brand: '点石数码', type: '制作幕后', title: '敢为人先', titleEn: 'Making Of · Dare to Lead', category: 'COMMERCIAL', video: siteAsset('/portfolio-media/dian-shi-making-of.mp4'), poster: siteAsset('/portfolio-media/dian-shi-making-of.jpg'), year: '2024 年', role: '剪辑师 / 编导', description: '此片是 2024 年深圳市形象宣传片《敢为人先》的幕后制作花絮。\n\n整体制作花费 3 个月时间，修改了 22 个版本，目的是整体展示正片制作的完整流程。此片也是最能体现剪辑节奏、结构判断与叙事能力的代表剪辑作品',
  },
  {
    slug: 'dr-roy-short-video-series', number: '03', brand: 'Dr.Roy', type: '短视频', title: '健康科普系列', titleEn: 'Health Explainer Series', category: 'COMMERCIAL', video: siteAsset('/portfolio-media/drroy-fructose.mp4'), poster: siteAsset('/portfolio-media/drroy-fructose.jpg'), year: '2025年', role: '剪辑师 / 编导', description: '以清晰结构和视觉节奏承载复杂健康知识的短视频系列。',
    series: [
  ...
  {
    slug: 'haifeisi-traceability', number: '04', brand: '海飞丝 × 屈臣氏', type: '短视频', title: 'HWB“十万公里”溯源之旅', titleEn: 'HWB 100,000 KM Traceability Journey', category: 'COMMERCIAL', video: siteAsset('/portfolio-media/haifeisi-traceability.mp4'), poster: siteAsset('/portfolio-media/haifeisi-traceability.jpg'), year: '2023年', role: '剪辑师、摄影师', description: '此片是由屈臣氏与宝洁合作的一支溯源短视频，是在新加坡拍摄的。\n\n视频目的是展示海飞丝总部的溯源，并进行一些产品展示。整体流程以主持人完成一个任务为线索，主要需要以板块化、结构化的方式去展示各个环节，突出每个环节的重点',
  },
  {
    slug: 'nanyue-museum', number: '05', brand: '南越王博物院', type: '宣传片', title: '文物展示', titleEn: 'Artifact Exhibition Film', category: 'COMMERCIAL', video: siteAsset('/portfolio-media/nanyue-museum.mp4'), poster: siteAsset('/portfolio-media/nanyue-museum.jpg'), year: '2023年', role: '剪辑师', description: '此片是南越王博物院的一个文物展示样片。\n\n我们想要以一种科技感的氛围去呈现每个文物，所以用 AE（After Effects）做了一些文物的 3D投息效果。',
  },
  {
    slug: 'pengcheng-new-energy', number: '06', brand: '鹏城新能源', type: '宣传片', title: '出海', titleEn: 'Going Global', category: 'COMMERCIAL', video: siteAsset('/portfolio-media/pengcheng-new-energy.mp4'), poster: siteAsset('/portfolio-media/pengcheng-new-energy.jpg'), year: '2025年', role: '剪辑师 / 编导', description: '此片是深圳市鹏程新能源公司的一个公司形象宣传片。因为客户是面向海外的，所以整体以英文形式去展示公司的整体项目、产品以及业务，最后还展现了公司的愿景。这是一个 Demo 版本',
  },
  {
    slug: 'ruitake-alarm-tvc', number: '07', brand: '锐塔克', type: 'TVC', title: '电子闹钟', titleEn: 'Alarm Clock TVC', category: 'COMMERCIAL', video: siteAsset('/portfolio-media/ruitake-tvc-alarm.mp4'), poster: siteAsset('/portfolio-media/ruitake-tvc-alarm.jpg'), year: '2024年', role: '剪辑师', description: '此片是深圳晓谷公司锐塔克品牌的一个电子闹钟 TVC 项目。\n\n讲的是一个陪伴的故事：电子闹钟从小陪到大，经历过各种事情，但它始终在那里。整体要突出一个比较偏成长、温和的情绪氛围。',
  },
  {
    slug: 'guangdong-tv-pet-toilet', number: '08', brand: '广东卫视', type: '电视栏目', title: '爱宠笑园—机场宠物厕所', titleEn: 'Pet Toilet at the Airport', category: 'TV & FILM', video: siteAsset('/portfolio-media/guangdong-tv-pet-toilet.mp4'), poster: siteAsset('/portfolio-media/guangdong-tv-pet-toilet.jpg'), year: '2023年', role: '剪辑师 / 编导', description: '这是一档在广东卫视播放的电视栏目《爱宠笑园》，栏目围绕宠物话题讲述一个个有趣生动的故事，这期节目围绕机场宠物厕所展开，展现了白云机场的人性化设施',
  },
  {
    slug: 'shein-anniversary', number: '09', brand: '希音', type: '活动快剪', title: '周年庆', titleEn: 'Anniversary Event Recap', category: 'COMMERCIAL', video: siteAsset('/portfolio-media/shein-anniversary.mp4'), poster: siteAsset('/portfolio-media/shein-anniversary.jpg'), year: '2023年', role: '剪辑师 / 编导', description: '这是广州希音的周年庆活动快剪，剪辑用时3小时，在海量素材中找出线索是挑战',
  },
  {
    slug: 'yuanclub-ar-game', number: '10', brand: 'YuanClub', type: '宣传片', title: '元宇宙 AR 游戏冲击波', titleEn: 'Metaverse AR Game Impact', category: 'COMMERCIAL', video: siteAsset('/portfolio-media/yuanclub-ar-game.mp4'), poster: siteAsset('/portfolio-media/yuanclub-ar-game.jpg'), year: '2024年', role: '剪辑师 / 编导', description: '此片是AR游戏冲击波的宣传片，除了赛场，还有多个场景的交叉蒙太奇叙事。在激烈的游戏比赛中插入观众反应突出现场的热情。',
  },
  {
    slug: 'nanshan-biomedical-investment', number: '11', brand: '南山区卫健委', type: '宣传片', title: '南山生物医药投资成果', titleEn: 'Nanshan Biomedical Investment Achievements', category: 'COMMERCIAL', video: siteAsset('/portfolio-media/nanshan-biomedical-investment.mp4'), poster: siteAsset('/portfolio-media/nanshan-biomedical-investment.png'), year: '2024 年', role: '剪辑师', description: '这条影片是我作为剪辑师，去展示南山区 2024 年生物医药投资成果。整体耗时 7 天，修改了 12 个版本。',
  },
];
  ...
  return (
    <main className={`site-shell ${pageReady ? 'is-ready' : 'is-loading'}`} onWheel={(event) => { event.preventDefault(); handleWheel(event.deltaY); }} onTouchStart={(event) => { const point = event.touches[0]; touchStart.current = point ? { x: point.clientX, y: point.clientY } : null; }} onTouchEnd={(event) => {
      ...
      <section className="work-stage" aria-label="作品浏览区">
        ...
        {outgoingProject && <div className={`hero-title hero-title-right hero-title-exit ${motionDirection === 1 ? 'exit-down' : 'exit-up'} tone-${activeIndex % 4}`} aria-hidden="true"><span>{outgoingProject.heroLabel ?? outgoingProject.brand}</span></div>}
        <div className={`hero-title hero-title-left hero-title-enter ${motionDirection === 1 ? 'enter-from-bottom' : 'enter-from-top'} tone-${activeIndex % 4}`} key={`left-${motionKey}`} aria-hidden="true"><span>{language === 'zh' ? activeProject.title : activeProject.titleEn}</span></div>
        <div className={`hero-title hero-title-right hero-title-enter ${motionDirection === 1 ? 'enter-from-top' : 'enter-from-bottom'} tone-${activeIndex % 4}`} key={`right-${motionKey}`} aria-hidden="true"><span>{activeProject.heroLabel ?? activeProject.brand}</span></div>
      ...
      {detailOpen && <section className="detail-panel" aria-label="项目详情"><div className="detail-video-wrap"><video ref={videoRef} key={currentVideo} src={currentVideo} poster={currentPoster} autoPlay muted={muted} loop playsInline controls preload="metadata" /></div><aside className="detail-copy"><div className="detail-header"><span>{Number(activeProject.number)} / {projects.length}</span><button className="detail-close" onClick={() => setDetailOpen(false)} aria-label="关闭项目详情">关闭</button></div><div className="detail-content"><p className="detail-kicker">{activeProject.type} / {categoryZh[activeProject.category]}</p><h2>{activeProject.title}</h2><p className="detail-subtitle">{activeProject.brand}</p><div className="detail-credit"><span>我的职责</span><strong>{activeProject.role}</strong></div><div className="detail-credit"><span>品牌方</span><strong>{activeProject.brand}</strong></div><div className="detail-credit"><span>年份</span><strong>{activeProject.year}</strong></div>{activeProject.tools && <div className="project-tools"><span className="detail-section-label">使用工具 / WORKFLOW</span><div className="project-tools-grid">{activeProject.tools.map((tool) => <div key={tool.label}><span>{tool.label}</span><strong>{tool.value}</strong></div>)}</div></div>}<p className="detail-description">{activeProject.description}</p>{activeProject.process && <section className="process-showcase"><div className="process-heading"><span className="detail-section-label">创作过程 / PROCESS</span>{activeProject.dossier && <a href={activeProject.dossier} target="_blank" rel="noreferrer">查看完整 PDF ↗</a>}</div><div className="process-grid">{activeProject.process.map((item, index) => <figure className="process-card" key={item.title}><img src={item.image} alt={`${activeProject.title}创作过程：${item.title}`} loading="lazy" /><figcaption><span>{String(index + 1).padStart(2, '0')} / {item.title}</span><small>{item.caption}</small></figcaption></figure>)}</div></section>}{activeProject.series && <div className="series-picker"><span>系列视频 / {String(activeProject.series.length).padStart(2, '0')}</span>{activeProject.series.map((clip, index) => <button key={clip.title} className={index === seriesIndex ? 'series-active' : ''} onClick={() => { setSeriesIndex(index); setPlaying(true); }}>{String(index + 1).padStart(2, '0')}　{clip.title}</button>)}</div>}</div><div className="detail-nav"><button onClick={() => moveProject(-1)}>← 上一个项目</button><button onClick={() => moveProject(1)}>下一个项目 →</button></div></aside></section>}
